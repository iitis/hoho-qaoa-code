...
function sparse_up_1qubit!(v::Vector{ComplexF64}, n::Int, k::Int, angle::Float64, d::Dict)
...
function sparse_up_1qubit_special!(v::Vector{ComplexF64}, n::Int, k::Int, d::Dict)
    fill!(v, zero(ComplexF64))
    @inbounds v[d[k]["topright"]] .= ComplexF64(-1im)
    SparseMatrixCSC{ComplexF64,Int32}(2^n, 2^n, d["colptr"], d[k]["rowval"], v)
end

function sparse_up_1qubit_der_dag!(
    v::Vector{ComplexF64},
    n::Int,
    k::Int,
    angle::Float64,
...
function sparse_up_1qubit_x!(v::Vector{ComplexF64}, n::Int, k::Int, d::Dict)
    fill!(v, zero(ComplexF64))
    @inbounds v[d[k]["topright"]] .= ComplexF64(1.0)
    SparseMatrixCSC{ComplexF64,Int32}(2^n, 2^n, d["colptr"], d[k]["rowval"], v)
end
